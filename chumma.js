    ...
    $('#verify').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.exists) {
                    $('#verification-result').html('ID is already in use.');
                } else {
                    $('#verification-result').html('ID is available.');
                }
            },
            error: function(response) {
                $('#verification-result').html('Error: ' + response.error);
            }
        });
    });
});
...
function onClick6() {
    ...
    if (loggedIn === 'true') {
        // If logged in, change the button action to redirect to the profile page
        loginButton.href = 'profile.htm';
    } else {
        // If not logged in, add an event listener to show the login window
