let xhrManageOrders = new XMLHttpRequest();
xhrManageOrders.open("GET", "displayfix.php", true);
xhrManageOrders.send();

xhrManageOrders.onreadystatechange = function() {
    if (xhrManageOrders.readyState === 4 && xhrManageOrders.status === 200) {
        let data = JSON.parse(xhrManageOrders.responseText);
        displayProducts2(data, "my-orders-content");
    }
};

function displayProducts2(products, containerId) {
